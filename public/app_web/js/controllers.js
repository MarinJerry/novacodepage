 ...
 */
 ...
 /**
  * Security - controller
  * Contains several global data used in security view
  * 
  */
 
  function SecurityCtrl($scope, ServAuth, $timeout, $location, SweetAlert,  notify){
    notify.config({
        duration: '5000'
    });
    ...
    $scope.getUsers = function(){
        try {
            ...
            var promise = ServAuth.getUsers();
            promise.then(
                function (response) {
                    try {
                        // console.info(response);
                        if(response !== null){
                            $scope.users = b64_to_utf8(response);
                        }
                    }catch (e) {
                ...
                function (response) {
                    try {
                        ...
                        notify({
                            message:'Error! El Servidor no encontró datos solicitados!',
 ...
 function LoginCtrl($scope, ServAuth, $timeout, $location, SweetAlert) {
    ...
    $scope.Login = function () {
        try {
            ...
            var promise = ServAuth.Authenticate(UserLogIn);
                ...
                function (response) {
                    ...
                    //    console.log(response);
                        if(response !== false && response.id > 0){
                            ...
                            sessionStorage.setItem('session_profile', response.profile_id);
                            sessionStorage.setItem('session_user_name', response.name);
                            
                            $timeout($location.path('/dashboards_top/dashboard_4'), 3000);
                        }else{
                            SweetAlert.swal({
                                title: "Lo Sentimos",
  ...
  function BlogCtrl($scope, ServBlog, $timeout, $location, SweetAlert,  notify){
    ...
    $scope.InitObjects = function () {
        try {
            ...
            $scope.oArticle = {
                ...
                "created_at": FechaActual(),
                "update_at": FechaActual(),
                "create_user": sessionStorage.getItem("sessionid"),
                "update_user": sessionStorage.getItem("sessionid"),
                "image": "assets/img/articulos/"
            };
    ...
    $scope.getArticles = function(){
        try {
            ...
            var promise = ServBlog.getArticles();
            promise.then(
                function (response) {
                    try {
                        
                        if(response !== null){
                            $scope.articles = b64_to_utf8(response);
                            // console.info($scope.articles);
                        }
                    }catch (e) {
                        console.error(e.message);
                    }
    ...
    $scope.getArticle = function(id){
        try {
            ...
            var promise = ServBlog.getArticle(id);
            promise.then(
                function (response) {
                    try {
                        // console.info(response);
                        if(response !== null){
                            $scope.oArticle = response;
                        }
                    }catch (e) {
                        console.error(e.message);
                    }
    ...
    $scope.crearArticle = function(){
        try {
            var promise = ServBlog.putArticle($scope.oArticle);
            promise.then(
                function (response) {
                    try {
                        alert(response.data);
